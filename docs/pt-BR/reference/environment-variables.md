...
| `ASTRADNS_ENGINE_TYPE` | `unbound` | Engine DNS a ser utilizado: `unbound`, `coredns` ou `powerdns` |
...
| Variavel | Padrao | Descricao |
|----------|--------|-----------|
| `ASTRADNS_ENGINE_TYPE` | `unbound` | Tipo de engine para renderizacao de config: `unbound`, `coredns` ou `powerdns` |
...
| `ENABLE_POOL_UNIQUENESS_WEBHOOK` | `false` | Habilitar o webhook de validacao que impoe um unico pool por namespace |
...
| `ENABLE_POOL_UNIQUENESS_WEBHOOK` | `webhook.enabled` | Deployment do operator |
